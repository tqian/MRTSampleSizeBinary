...
tau_t <- rep(0.8, total_dp) # expected availability E(I_t) for t = 1,...,total_dp
...
mu0_t <- exp(g_t %*% alpha) # E(Y_{t+1} = 1 | I_t = 1, A_t = 0) for t = 1,...,total_dp
...
test_that(
  "check TQ's sample",
  {
    expect_equal(MRT_bin_samplesize.F(tau_t, f_t, g_t, beta, alpha, p_t, gamma, b),
  calculate_mrt_bin_samplesize_f(tau_t, f_t, g_t, beta, alpha, p_t, gamma, b))
  }
)
