...
author: "Thabat Dahdoul, Eliot Wong-Toi, Tianchen Qian"
...
The function to calculate the sample size is `calculate_mrt_bin_samplesize_f()`.
The function to calculate the sample size is `calculate_mrt_bin_power_f()`.
Because the syntax for their use are similar, here we illustrate the use of
`calculate_mrt_bin_samplesize_f()`.

The function `calculate_mrt_bin_samplesize_f()` takes in the following arguments:
...
- `g_t` and `alpha`: They characterize the success probability null curve $E(Y_{t+1} \mid A_t = 0, I_t = 1)$ for $1 \leq t \leq m$, where `g_t` is a matrix of size $m \times q$ and `alpha` is a vector of length $q$, and $q$ is the degrees of freedom for the success probability null curve. Specifically, $E(Y_{t+1} \mid A_t = 0, I_t = 1)$ equals `g_t[t, ] %*% alpha` for each $1 \leq t \leq m$. Usually the first column of `g_t` is a column of 1's. In addition, it is required that the linear column span of `g_t` contains `f_t[, j] * p_t` for each $1 \leq j \leq p$ for the sample size calculation result to be accurate; `calculate_mrt_bin_samplesize_f()` checks for this and if not satisfied the function will output a warning.
...
`calculate_mrt_bin_samplesize_f()`, `calculate_mrt_bin_power_f()`,
...
### Example use of `calculate_mrt_bin_samplesize_f()`
...
### Example use of `calculate_mrt_bin_power_f()`
